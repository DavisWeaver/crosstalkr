...
```{r filter}
g <- gfilter(method = "value", cache = NULL, val=exp, val_name = "expression", 
             use_ppi = TRUE, min_score= 800,desc = TRUE,n=100)
length(igraph::V(g))
head(igraph::get.vertex.attribute(g, name = "expression"))
```
## Node Scoring

Next, I'll use gfilter again to score this smaller subgraph according to the betweenness centrality and return only the top 5 proteins.
...
out <- gfilter.ct(seeds = names(igraph::V(g)), use_ppi = TRUE, min_score=800, 
                return_df=TRUE, cache = NULL, seed_name = "vignette")
...
Next, we will re-analyze this data using a different combination of analytic steps to illustrate some more of crosstalkr's capabilities. 
...
```{r reduction2}
g <- gfilter(use_ppi=TRUE, cache = NULL, n = 2000, igraph_method = igraph::degree, val_name = "degree")
...
It is possible to modify these types of analysis to reduce the likelihood of finding  proteins that are involved in the generic stress response.
